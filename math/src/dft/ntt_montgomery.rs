...
impl Ntt {
    pub fn new(q: Prime, ring_size: usize) -> Self {
        ...
        let [twiddle_bo, twiddle_inv_bo] = [g, q.inv(&g).unwrap()]
            .map(|b| Vec::from_iter(q.powers(&b).take(ring_size).map(|v| q.montgomery(v))))
            .map(bit_reverse);
        let n_inv = q.montgomery(q.inv(&(ring_size as u64)).unwrap());
        Self {
            q,
            q_twice: *q << 1,
            q_quart: *q << 2,
    ...
    #[inline(always)]
    fn dit<const LAZY: bool>(&self, a: &mut u64, b: &mut u64, t: u64) {
        debug_assert!(*a < self.q_quart);
    ...
    #[inline(always)]
    fn dif<const LAZY: bool>(&self, a: &mut u64, b: &mut u64, t: u64) {
